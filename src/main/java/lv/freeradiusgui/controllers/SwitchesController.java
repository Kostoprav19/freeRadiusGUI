...
public class SwitchesController {
  ...
  @RequestMapping(
    value = { Views.SWITCH_LIST, Views.SWITCH },
  ...
  public String showSwitch(@PathVariable("id") Integer aSwitchId, Model model) {
  ...
  @RequestMapping(
    value = Views.SWITCH + "/delete/{id}",
    method = RequestMethod.GET
  )
  public String deleteSwitch(
    @PathVariable("id") Integer aSwitchId,
    ...
    if (switchService.delete(aSwitch)) {
      redirectAttributes.addFlashAttribute(
        "msg",
        "Switch '" + aSwitch.getName() + "' successfully deleted."
      );
      redirectAttributes.addFlashAttribute("msgType", "success");
    } else {
      redirectAttributes.addFlashAttribute(
        "msg",
        "Error deleting switch '" + aSwitch.getName() + "'."
      );
  ...
  public String storeSwitch(
    @ModelAttribute("aSwitch") @Validated Switch aSwitch,
    ...
    redirectAttributes.addFlashAttribute(
      "msg",
      "Switch '" + aSwitch.getName() + "' successfully saved."
    );
    redirectAttributes.addFlashAttribute("msgType", "success");
    return "redirect:/" + Views.SWITCH_LIST;
  }
  ...
  public String reloadSwitches(final RedirectAttributes redirectAttributes) {
    if (switchService.reloadFromConfig()) {
      redirectAttributes.addFlashAttribute(
        "msg",
        "Successfully loaded 'clients.conf' file."
      );
      redirectAttributes.addFlashAttribute("msgType", "success");
    } else {
      redirectAttributes.addFlashAttribute(
        "msg",
        "Error loading 'clients.config' file."
      );
      redirectAttributes.addFlashAttribute("msgType", "danger");
    }
    return "redirect:/" + Views.SWITCH_LIST;
  ...
  @RequestMapping(
    value = Views.ADMIN + "/writeClients",
    method = RequestMethod.GET
  )
  public String writeSwitches(final RedirectAttributes redirectAttributes) {
    if (switchService.writeToConfig()) {
      if (serverService.restartFreeradius()) {
        ...
        redirectAttributes.addFlashAttribute(
          "msg",
          "Changes saved to 'clinets.conf' file, but failed to restart freeradius service."
        );
        redirectAttributes.addFlashAttribute("msgType", "danger");
      }
    } else {
      redirectAttributes.addFlashAttribute(
        "msg",
        "Error writing to 'clients.conf' file."
      );
      redirectAttributes.addFlashAttribute("msgType", "danger");
    }
    return "redirect:/" + Views.SWITCH_LIST;
