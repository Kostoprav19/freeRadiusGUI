  ...
  implements FileService<Device>
{
  ...
  public boolean saveListToFile(List<Device> list) {
    ...
    for (Device device : list) {
      String accessStr = (device.getAccess() == Device.ACCESS_ACCEPT)
        ? "Accept"
        : "Reject";
    ...
    //        Reply-Message = "ansis-imac-ethernet2, Access-Accept, MAC:%{User-Name},
    // Connection: %{Connect-Info}, Port: %{NAS-Port}, Switch IP: %{NAS-IP-Address}, Switch
    // Name: %{NAS-Identifier}"
  ...
  private String detectType(String name) {
    ...
    if (
      (name.toUpperCase().contains("TIMECAPSULE")) ||
